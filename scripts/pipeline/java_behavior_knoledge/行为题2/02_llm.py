...
{{
  ...
  "metadata": {{
    ...
    try:
        ...
        print(f"题目处理失败 ({raw_item.get('category', '未知分类')}): {e}")
        return None


if __name__ == "__main__":
    PROJECT_ROOT = r"D:\大三作业\大三下作业\project"
    input_file = r"D:\大三作业\大三下作业\project\data\processed\行为题\行为题2.json"
    output_file = os.path.join(PROJECT_ROOT, "data", "output", "行为题", "行为题2.json")

    # 【安全点3：自动创建输出目录】防止 FileNotFoundError
    ...
    try:
        ...
        # 无论发生什么情况，都会执行这里的保存代码，抢救已经跑完的数据
        if len(final_dataset) > 0:
            with open(output_file, 'w', encoding='utf-8') as f:
                json.dump(final_dataset, f, ensure_ascii=False, indent=2)
            print(f"\n数据已安全保存！本次成功提取了 {len(final_dataset)} 道题目。")
            print(f"存放路径: {output_file}")
        else:
            print("\n没有提取到任何数据，未生成文件。")